...
if action == "getbookingdetails":
    ...
    sql = "select left(booking.booking_date,10) booking_date,customer.name,product.product_name,booking.quantity,booking.price,left(booking.deli_date,10) deli_date from customer INNER JOIN (booking INNER JOIN product ON product.product_id=booking.product_id) ON customer.cust_id=booking.cust_id where booking.deli_date between %s and %s"
