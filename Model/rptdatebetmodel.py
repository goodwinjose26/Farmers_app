...
if action == "getbookingdetails":
    ...
    sql = "select left(booking.booking_date,10) booking_date,customer.name,product.product_name,booking.quantity,booking.price from customer INNER JOIN (booking INNER JOIN product ON product.product_id=booking.product_id) ON customer.cust_id=booking.cust_id where booking.booking_date between %s and %s and (booking.status=6 or booking.status=5 or booking.status=1 or booking.status=20  )"
