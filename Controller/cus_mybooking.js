...
function GetBooking() {
    ...
    console.log(data)
    $.post("Model/cus_mybooking.py", data, function (res) {
        res = JSON.parse(res)
        console.log(res)
        if (res != null) {
            $("#tb").show()
            $.each(res, function (k, v) {
                s = '<tr><td>' + v.booking_date + '</td>' +
                    ...
                    '<td>' + v.quantity + '</td>' +
                    '<td>' + v.price + '</td>' +
                    '<td>' +  "<a href='#' id='"+v.booking_id+"' class='btn  btn-sm btn-outline-danger del'>Cancel Payment</a> &nbsp;&nbsp;&nbsp;"+'</td>' 
            ...
            $('.del').click(function (e) {
                e.preventDefault()
                if (confirm("Delete item from booking ? ")) {
                    v = $(this).attr('id')
                    data = { "id": v }
